...
import { DroidAgentParser } from "../../../core/parsers/droid-agent-parser.js";
import { mkdir, writeFile, rm } from "node:fs/promises";
import { join } from "node:path";
import { tmpdir } from "node:os";

describe("DroidAgentParser", () => {
  let testDir: string;

  beforeEach(async () => {
    testDir = join(tmpdir(), `opito-test-droid-agent-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
with multiple lines`;
    ...
    const parser = new DroidAgentParser(testDir);
    ...
    expect(agents[0].frontmatter.model).toBe("claude-sonnet-4-20250514");
    expect(agents[0].frontmatter.tools).toEqual(["Read", "Write"]);
    expect(agents[0].frontmatter.userInvocable).toBe(true);
    expect(agents[0].frontmatter.disableModelInvocation).toBe(false);
  });
  ...
  test("should return empty array for non-existent directory", async () => {
    const parser = new DroidAgentParser(join(testDir, "non-existent"));
    const agents = await parser.parseAll();
...
Content`;
  ...
  test("should handle agent without user-invocable field", async () => {
    const content = `---
name: no-invocable-agent
description: Agent without invocable
---

Content`;

    await writeFile(join(testDir, "no-invocable-agent.md"), content);

    const parser = new DroidAgentParser(testDir);
    const agents = await parser.parseAll();

    expect(agents).toHaveLength(1);
    expect(agents[0].frontmatter.userInvocable).toBeUndefined();
  });

  test("should handle agent without disable-model-invocation field", async () => {
    const content = `---
name: no-disable-agent
description: Agent without disable
---

Content`;

    await writeFile(join(testDir, "no-disable-agent.md"), content);

    const parser = new DroidAgentParser(testDir);
    const agents = await parser.parseAll();

    expect(agents).toHaveLength(1);
    expect(agents[0].frontmatter.disableModelInvocation).toBeUndefined();
  });
...
Content`;
  ...
  test("should parseAgentFile directly", async () => {
    const content = `---
name: direct-parse
description: Direct parse test
---

Direct content`;

    const parser = new DroidAgentParser(testDir);
    const agent = parser.parseAgentFile(content, "direct-parse.md", "/test/path.md");

    expect(agent).not.toBeNull();
    expect(agent!.name).toBe("direct-parse");
    expect(agent!.description).toBe("Direct parse test");
    expect(agent!.content).toBe("Direct content");
  });

  test("should return null for invalid content in parseAgentFile", async () => {
    const parser = new DroidAgentParser(testDir);
    
    expect(parser.parseAgentFile("No frontmatter", "test.md", "/test.md")).toBeNull();
    expect(parser.parseAgentFile("---", "test.md", "/test.md")).toBeNull();
    expect(parser.parseAgentFile("---", "test.md", "/test.md")).toBeNull();
  });
});
