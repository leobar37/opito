...
import { ClaudeAgentParser } from "../../../core/parsers/claude-agent-parser.js";
import { mkdir, writeFile, rm } from "node:fs/promises";
import { join } from "node:path";
import { tmpdir } from "node:os";

describe("ClaudeAgentParser", () => {
  let testDir: string;

  beforeEach(async () => {
    testDir = join(tmpdir(), `opito-test-claude-agent-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
with multiple lines`;
    ...
    const parser = new ClaudeAgentParser(testDir);
    ...
    expect(agents[0].frontmatter.tools).toEqual(["Read", "Write", "Bash"]);
  });
  ...
  test("should return empty array for non-existent directory", async () => {
    const parser = new ClaudeAgentParser(join(testDir, "non-existent"));
    const agents = await parser.parseAll();
