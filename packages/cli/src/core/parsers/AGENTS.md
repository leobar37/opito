...
## OVERVIEW
Parsers read markdown files from different AI coding assistants (Claude, OpenCode, Droid) and convert them to a standardized internal format. Each parser handles the specific frontmatter format and file organization of its provider.
...
- **Frontmatter format varies by provider** — Claude: simple YAML, Droid: structured metadata
- **File extensions vary** — `.md` (Claude/OpenCode/Droid), `.md` with special headers (Droid)
