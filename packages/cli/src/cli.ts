...
import type { Provider, Scope } from "@opito/core";
import { unifiedSyncCommand, legacySyncCommand } from "./commands/sync.js";
import {
  syncCopilotCommand,
  syncDroidCommand,
  syncSkillsCommand,
...
cli
  ...
  .option(
    "-t, --target <target>",
    "Target provider (claude, opencode, copilot, droid)"
  )
  .option(
    "-l, --local",
  ...
  .example(
    "opito sync copilot -t claude -l     # Sync copilot → claude locally"
  )
...
cli
  .command(
    "sync-copilot",
    "Sync commands to/from VS Code Copilot (deprecated, use: opito sync [source] copilot)"
  )
  .option("--dry-run", "Show what would be synced without making changes")
  .option("--force", "Skip backup and overwrite existing commands")
  ...
  .action(
    async (options: {
      ...
      try {
        logger.warning(
          "sync-copilot is deprecated. Use: opito sync [source] copilot"
        );
        const config = await configManager.load();
...
cli
  .command(
    "sync-droid",
    "Sync Claude commands to Droid (deprecated, use: opito sync claude droid)"
  )
  .option("--dry-run", "Show what would be synced without making changes")
  .option("--force", "Skip backup and overwrite existing commands")
  .option("--filter <commands>", "Comma-separated list of commands to sync")
  .action(
    async (options: { dryRun?: boolean; force?: boolean; filter?: string }) => {
      try {
        logger.warning(
          "sync-droid is deprecated. Use: opito sync claude droid"
        );
        const config = await configManager.load();
...
cli.command("dashboard", "Open OPITO dashboard (TUI)").action(async () => {
  try {
